...
class ProjectContextServer {
  constructor() {
    this.server = new Server(
      { name: 'em-team-project-context', version: '1.0.0' },
      { capabilities: { tools: {} } }
    );
      ...
      tools: [
        ...
        },
        {
          ...
          inputSchema: {
            type: 'object',
            properties: {
              type: { type: 'string', enum: ['all', 'dependencies', 'devDependencies'], default: 'all' },
            },
          },
        },
        {
        ...
        },
        {
          ...
          description: 'Analyze codebase metrics (LOC, files, by extension)',
          inputSchema: {
            type: 'object',
            properties: {
  ...
  async getProjectStructure(args) {
    ...
    return {
      content: [{
        type: 'text',
        text: JSON.stringify({ root: this.projectRoot, tree: tree.join('\n') }, null, 2),
      }],
    };
  }
  ...
  async getDependencies(args) {
    ...
    const result = { name: packageJson.name, version: packageJson.version };
  ...
  async getAgentConfig(args) {
    ...
    if (fs.existsSync(settingsPath)) {
      config.settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
    }
    if (fs.existsSync(localSettingsPath)) {
      config.localSettings = JSON.parse(fs.readFileSync(localSettingsPath, 'utf8'));
    }
    ...
    const analyzeDirectory = (dir, stats = { files: 0, lines: 0, byExtension: {} }) => {
      ...
      items.forEach(item => {
        ...
        if (stat.isDirectory()) {
          ...
          if (!includeTests && ['tests', '__tests__', 'test'].includes(item)) return;
          analyzeDirectory(itemPath, stats);
        } else if (stat.isFile()) {
          const ext = path.extname(item);
          if (['.ts', '.js', '.tsx', '.jsx', '.json', '.md', '.py', '.go', '.rs'].includes(ext)) {
            stats.files++;
      ...
      content: [{
        ...
        text: JSON.stringify({
          summary: {
            ...
            averageLinesPerFile: stats.files > 0 ? Math.round(stats.lines / stats.files) : 0,
          },
  ...
  async getRecentChanges(args) {
    ...
    try {
      ...
      return {
        content: [{
          type: 'text',
          text: JSON.stringify({ branch, recentCommits: commitList }, null, 2),
        }],
      };
    } catch (error) {
      return {
        content: [{ type: 'text', text: 'Git repository not found or git command failed' }],
        isError: true,
      };
    }
  }
  ...
  async run() {
    ...
    console.error('EM-Team Project Context MCP Server running on stdio');
  }
}
